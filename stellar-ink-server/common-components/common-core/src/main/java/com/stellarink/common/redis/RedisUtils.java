...
public class RedisUtils {
    ...
    /**
     * 原子增加整数计数器，并仅在键首次创建时设置有效期。
     *
     * <p>适合失败次数、短期计数等滑动窗口场景；后续自增不会刷新窗口，避免攻击者通过持续请求延长计数器寿命。
     */
    public long increment(String key, long delta, Duration ttl) {
        ...
        Long result = redisTemplate.execute(INCREMENT_WITH_TTL_SCRIPT, List.of(safeKey),
                Long.toString(delta), Long.toString(safeTtl.toMillis()));
        if (result == null) {
            throw new IllegalStateException("Redis 带有效期计数器自增未返回结果，key=" + key);
        }
        return result;
    }
